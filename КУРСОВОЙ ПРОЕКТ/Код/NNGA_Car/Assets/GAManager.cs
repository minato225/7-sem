...
public class GAManager : MonoBehaviour
{
    ...
    {
        var bestIndivid = population.Max(x => x.fitness);
        var averageIndivid = population.Average(x => x.fitness);
        File.AppendAllText("best.txt", bestIndivid.ToString() + ", ");
        File.AppendAllText("average.txt", averageIndivid.ToString() + ", ");
        genePool.Clear();
        currentGeneration++;
        naturallySelected = 0;
        SortPopulation();
